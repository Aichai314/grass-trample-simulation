...
void main()
{
	...
	// On ignore fragment.normal et on utilise les dérivées spatiales pour trouver la pente
    vec3 dpdx = dFdx(fragment.position);
    vec3 dpdy = dFdy(fragment.position);
    vec3 N = normalize(cross(dpdx, dpdy));

	// Sécurité : on s'assure que la normale pointe bien vers le haut/vers la caméra
